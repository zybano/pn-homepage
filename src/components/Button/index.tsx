...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-lg text-base font-semibold ring-offset-white transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98]",
  {
    variants: {
      variant: {
        primary: "text-white shadow-[0px_4px_7px_rgba(87,104,253,0.35)] hover:opacity-90",
        secondary: "bg-slate-100 text-[#040523] border border-brand-border hover:bg-slate-200",
        outline: "border border-brand-border bg-transparent text-brand-navy hover:bg-brand-bg",
  ...
  ({ className, variant, size, ...props }, ref) => {
    ...
    return (
      <motion.button
        ...
        ref={ref}
        style={isPrimary ? {
          backgroundImage: 'linear-gradient(94.9deg, #040523 2.59%, #3B4FE8 105%)',
        } : undefined}
        whileHover={{ y: -2 }}
        whileTap={{ scale: 0.98 }}
        {...props}
